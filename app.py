...
"""
    ...
    try:
        ...
        if not client and not openai.api_key:
        ...
        return {
            'essay_type': essay_type.title(),
            'scores': {'ideas': 75, 'organization': 75, 'style': 75, 'grammar': 75},
            'score_reasons': {
                'ideas': 'Analysis temporarily unavailable.',
                'organization': 'Analysis temporarily unavailable.',
            ...
            'suggestions': [
                {
                    'type': 'General',
                    'text': 'Unable to provide detailed analysis at this time',
                    'reason': f'Error: {error_type} - {str(e)}'
                }
            ],
            'overall_feedback': 'Analysis temporarily unavailable. Please check your API key or internet connection.',
            'examples': {
                'ideas': [],
                'organization': [],
                'style': [],
            ...
            'analysis_id': None  # Indicate no database save occurred
        }
...
@app.route('/export/<int:essay_id>')
@login_required
def export_essay_file(essay_id):
    """Export essay as DOCX file"""
    conn = get_db_connection()
    cursor = conn.cursor()
    
    # Verify user owns essay
    if session['role'] == 'student':
        cursor.execute("SELECT title, content FROM essays WHERE id = %s AND user_id = %s", 
                      (essay_id, session['user_id']))
    else:
        cursor.execute("""
            SELECT e.title, e.content FROM essays e
            JOIN users u ON e.user_id = u.id
            JOIN student_teacher_assignments sta ON u.id = sta.student_id
            WHERE e.id = %s AND sta.teacher_id = %s
        """, (essay_id, session['user_id']))
